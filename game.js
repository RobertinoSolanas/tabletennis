...
function init() {
    ...
    setTimeout(function () {
        ...
        // Main menu is visible by default, no need to show it
    }, 1500);
...
function startGame(mode) {
    ...
    state.sets.right = 0;
    state.serveCount = 0;
    state.serving = 'right'; // Let robot serve first in PvE so player can see the ball
    updateUI();
...
function setupControls() {
    ...
    // --- Mouse/Touch input for human player ---
    canvas.addEventListener('mousemove', function (e) {
        if (state.mode !== 'pve') return;
        var rect = canvas.getBoundingClientRect();
        var nx = ((e.clientX - rect.left) / rect.width) * 2 - 1;  // -1 to 1
        var ny = -((e.clientY - rect.top) / rect.height) * 2 + 1; // -1 to 1 (inverted)
        humanMouse.x = nx * (TABLE_WIDTH / 2 + 1.0);
        humanMouse.y = Math.max(TABLE_HEIGHT + 0.1, TABLE_HEIGHT + 0.7 + ny * 1.5);
        // Track vertical speed for swing power
...
function updateHumanPlayer(dt) {
    ...
    var forwardZ = -TABLE_LENGTH / 2 + 0.3; // near the net edge of player's side
    var backZ = -TABLE_LENGTH / 2 - 0.6;     // default resting position
